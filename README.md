...
**Next.js · TypeScript · Node · Postgres · Supabase · Telephony · LLM Integration**
...
I run engineering and operations for a software agency in the Philippines. Day to day that means **multi-tenant SaaS in production**: CRMs that sales floors live in, telephony and billing platforms moving real money, marketplaces with two-sided payments, and internal tooling that has to be right the first time because a client is watching.

Most of what I do is **not greenfield**. It is inheriting a system, finding what is silently broken, and fixing it without taking the business offline. A lot of my best work is invisible: a payment path that stopped losing donations, an auth check that was never there, a billing rule that had been undercharging for months.

I also lead a small team, so I write specs and reviews as much as I write code.
...
Client work, so described by shape rather than name.

| System | Stack | Scale |
|---|---|---|
| **Multi-tenant sales CRM** | Express · Drizzle · Postgres · React · Twilio | A sales floor works in it daily. Pipeline, browser calling, SMS, lead intake from third-party feeds, sub-account role model with tenant isolation |
| **Telecom billing platform** | Node · Postgres · SIP · carrier APIs | Number provisioning, per-message and per-minute metering, CDR reconciliation, toll-fraud hardening after a live incident |
| **Healthcare booking SaaS** | Next.js · Supabase · Tailwind | Clinic dashboard and patient marketplace, OTP auth, appointment engine, SMS reminders, programmatic SEO |
| **Two-sided services marketplace** | Next.js · Supabase · Stripe Connect | Provider onboarding and payouts, booking flow with payment gating, in-app messaging, iOS app |
